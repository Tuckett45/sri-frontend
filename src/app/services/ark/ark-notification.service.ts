...
import {
  ...
  ArkNotificationTemplate,
  ArkNotificationLog,
  ArkBroadcastNotification,
  ArkNotificationFilters,
  ArkNotificationSummary,
...
/**
 * Service for managing ARK role-based notifications
 * 
...
export class ArkNotificationService {
  ...
  /**
   * Validate that a notification type belongs to the ARK domain
   * @param type Notification type to validate
   * @returns True if valid ARK notification type
   * @throws Error if type is invalid or belongs to ATLAS domain
   */
  validateNotificationType(type: string): boolean {
    const validArkTypes = Object.values(ArkNotificationType);
    
    if (!validArkTypes.includes(type as ArkNotificationType)) {
      throw new Error(`Invalid ARK notification type: ${type}. Type does not match ARK domain.`);
    }
  ...
  sendNotification(notification: Partial<ArkNotification>): Observable<ArkNotification> {
    ...
    // Validate notification type belongs to ARK domain
    ...
    return this.http.post<ArkNotification>(`${this.apiUrl}`, notification);
  }
  ...
  getNotificationsForUser(filters?: ArkNotificationFilters): Observable<ArkNotification[]> {
    ...
    return this.http.get<ArkNotification[]>(`${this.apiUrl}`, { params }).pipe(
  ...
  getNotificationById(notificationId: string): Observable<ArkNotification> {
    return this.http.get<ArkNotification>(`${this.apiUrl}/${notificationId}`).pipe(
      map(notification => {
  ...
  markAsRead(notificationId: string): Observable<ArkNotification> {
    return this.http.patch<ArkNotification>(`${this.apiUrl}/${notificationId}/read`, {});
  }
  ...
  getNotificationSummary(): Observable<ArkNotificationSummary> {
    ...
    return this.http.get<ArkNotificationSummary>(`${this.apiUrl}/summary`, { params });
  }
  ...
  configureNotificationPreferences(preferences: ArkNotificationPreferences): Observable<ArkNotificationPreferences> {
    ...
    return this.http.put<ArkNotificationPreferences>(`${this.apiUrl}/preferences`, preferences);
  }
  ...
  getNotificationPreferences(userId?: string): Observable<ArkNotificationPreferences> {
    ...
    return this.http.get<ArkNotificationPreferences>(`${this.apiUrl}/preferences/${targetUserId}`);
  }
  ...
  sendBroadcast(broadcast: Partial<ArkBroadcastNotification>): Observable<ArkBroadcastNotification> {
    ...
    return this.http.post<ArkBroadcastNotification>(`${this.apiUrl}/broadcast`, broadcast);
  }
  ...
  getNotificationLogs(filters?: ArkNotificationFilters): Observable<ArkNotificationLog[]> {
    ...
    return this.http.get<ArkNotificationLog[]>(`${this.apiUrl}/logs`, { params });
  }
  ...
  configureNotificationTemplates(template: ArkNotificationTemplate): Observable<ArkNotificationTemplate> {
    ...
    if (template.id) {
      ...
      return this.http.put<ArkNotificationTemplate>(`${this.apiUrl}/templates/${template.id}`, template);
    } else {
      // Create new template
      template.createdBy = user.id;
      template.createdAt = new Date();
      template.updatedAt = new Date();
      return this.http.post<ArkNotificationTemplate>(`${this.apiUrl}/templates`, template);
    }
  }
   ...
   */
  getNotificationTemplates(activeOnly: boolean = false): Observable<ArkNotificationTemplate[]> {
    if (!this.authService.isAdmin()) {
    ...
    return this.http.get<ArkNotificationTemplate[]>(`${this.apiUrl}/templates`, { params });
  }
  ...
  sendHighPriorityNotification(
    notification: Partial<ArkNotification>,
    channels: ArkNotificationChannel[] = [ArkNotificationChannel.Email, ArkNotificationChannel.InApp, ArkNotificationChannel.SMS]
  ): Observable<ArkNotification> {
    notification.priority = ArkNotificationPriority.Critical;
   ...
   */
  private applyMarketFilter(notifications: ArkNotification[]): ArkNotification[] {
    if (this.authService.isAdmin()) {
   ...
   */
  private canAccessNotification(notification: ArkNotification): boolean {
    if (this.authService.isAdmin()) {
      return true;
    }
