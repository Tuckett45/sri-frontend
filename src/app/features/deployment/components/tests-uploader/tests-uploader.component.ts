...
@Component({
  selector: 'ark-tests-uploader',
  standalone: true,
  imports: [CommonModule, FileUploadModule, ButtonModule],
  templateUrl: './tests-uploader.component.html',
  styleUrls: ['./tests-uploader.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class TestsUploaderComponent {
  ...
  protected async onUpload(event: FileUploadHandlerEvent) {
    ...
    const uploadedIds = await this.uploadFiles(files);
  ...
  private async uploadFiles(files: File[]): Promise<string[]> {
    // TODO: integrate Fluke upload pipeline
