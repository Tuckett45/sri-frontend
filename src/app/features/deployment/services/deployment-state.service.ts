...
export class DeploymentStateService {
  ...
  clear() {
    if (this.deploymentId) this.socket.leaveDeployment(this.deploymentId);
  ...
  private async ensureSocket(id: string) {
    ...
    this.socket.phaseAdvanced$.subscribe(e => {
    ...
    this.socket.checklistSaved$.subscribe(e => { if (e.deploymentId === id) {/* maybe toast or refresh phase */} });
    this.socket.evidenceAdded$.subscribe(e => { if (e.deploymentId === id) {/* update counts/badges */} });
    this.socket.punchUpdated$.subscribe(e => { if (e.deploymentId === id) {/* refresh punch list */} });
    this.socket.handoffSigned$.subscribe(e => { if (e.deploymentId === id) {/* UI signal */} });
    this.socket.handoffArchived$.subscribe(e => { if (e.deploymentId === id) {/* show archive link */} });
  }
